...
class CustomText1(nn.Module):
    def __init__(self):
        super(CustomText1, self).__init__()
        self.conv = ConvBlock1d(in_channels=200,
                                out_channels=256,
                                kernel=3,
                                stride=1,
                                pad=1,
        ...
        self.lstm = nn.LSTM(input_size=256,
                            ...
                            batch_first=True,
                            bidirectional=False)
        self.fc1 = FullyConnected(in_channels=256,
                                 out_channels=1,
                                 activation='sigmoid',
                                 normalisation=None)
        self.fc2 = FullyConnected(in_channels=256,
                                 out_channels=107,
                                 activation='global',
                                 normalisation=None)
    ...
    def forward(self, net_input):
        ...
        spk = self.fc2(x[:, -1, :].reshape(batch, -1))

        return mdd,spk, hidden_state[0][-1]
...
if __name__ == '__main__':
    model = CustomText1()
    pytorch_total_params_depaudionet = sum(p.numel() for p in model.parameters())
    print(pytorch_total_params_depaudionet)