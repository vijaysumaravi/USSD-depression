...
# text+CNN-LSTM
from exp_run.models_pytorch import CustomText1 as CustomText
from exp_run import word2vec_config as config
...
def forward(model, generate_dev, data_type):
    ...
    for data in generate_dev:
        ...
        with torch.no_grad():
            batch_output, _, batch_embd = get_output_from_model(model=model,
                                                 data=batch_data)
        counter += 1
        # Append data
    ...
    results_dict['embeddings'] = np.array(embeddings)
...
def create_model():
    ...
    if config.EXPERIMENT_DETAILS['FEATURE_EXP'] == 'mel':
        ...
        model = CustomRaw()
    elif config.EXPERIMENT_DETAILS['FEATURE_EXP'] == 'wav2vec2':
        model = CustomWav2vec2()
    elif config.EXPERIMENT_DETAILS['FEATURE_EXP'] == 'compare16':
        model = CustomComparE()
    elif 'text' in config.EXPERIMENT_DETAILS['FEATURE_EXP']:
        model = CustomText()
    if cuda:
        model.cuda()
    return model
...
def get_output_from_model(model, data):
    ...
    output_pred, spk_pred, out_embd = model(current_data)
    if cuda:
        output_pred = output_pred.cpu()
        spk_pred = spk_pred.cpu()
        out_embd = out_embd.cpu()

    return output_pred, spk_pred, out_embd
...
def train(model, workspace_files_dir):
    ...
    for (iteration, (batch, train_batch_loc, data_saver)) in enumerate(
        ...
        model.train()
        batch_output,speaker_output,_  = get_output_from_model(model=model,
                                             data=batch[0])
        batch_folders = batch[2]
        speaker_labels = torch.LongTensor(np.array([train_folders.index(val) for val in batch_folders]))
        ...
        train_loss_mdd = mu.calculate_loss(batch_output,
                                       ...
                                       batch_weights,
                                       gender_balance)
        train_loss_spk = mu.calculate_speaker_loss(speaker_output,
                                        speaker_labels)

        train_loss_spk =  -1 * config.EXPERIMENT_DETAILS['LOSS_ALPHA'] * train_loss_spk
