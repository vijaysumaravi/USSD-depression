...
import pandas as pd
from audio.rasta import rastaplp
...
def max_min_values(current_directory, win_size, hop_size, audio_paths,
    ...
    if config.EXPERIMENT_DETAILS['FEATURE_EXP'] =='wav2vec2':
        feat_dict = {spkID:featVec for spkID,featVec in kaldi_io.read_vec_flt_ark(config.WAV2VEC2_ARK) }
    if config.EXPERIMENT_DETAILS['FEATURE_EXP'] =='hubertLarge':
        feat_dict = {spkID:featVec for spkID,featVec in kaldi_io.read_vec_flt_ark(config.HUBERTLARGE_ARK) }
    for iterator, filename in enumerate(audio_paths):
        ...
        if config.EXPERIMENT_DETAILS['FEATURE_EXP'] =='wav2vec2' or config.EXPERIMENT_DETAILS['FEATURE_EXP'] =='hubertLarge':
            feat_vec = feat_dict[str(folder_number)]
            win_size = int(feat_vec.shape[0]/config.EXPERIMENT_DETAILS['FREQ_BINS'])
            if win_size > max_win:
                max_win = win_size
            if win_size < min_win:
                min_win = win_size
        if config.EXPERIMENT_DETAILS['FEATURE_EXP'] == "compare16":
            feat_path = config.COMPARE16_FOLDER+'/'+str(folder_number)+'_P_audio_data.csv'
    ...
    # length.
    # output = (audio // hop) + 1
    if config.EXPERIMENT_DETAILS['FEATURE_EXP'] =='wav2vec2' or config.EXPERIMENT_DETAILS['FEATURE_EXP'] =='hubertLarge' or config.EXPERIMENT_DETAILS['FEATURE_EXP'] =='compare16':
        total_windows_in_file_max = max_win
        total_windows_in_file_min = min_win
    else:
...
def create_database(labels, sample_rate, total_windows_in_file_max, max_value,
    ...
    if features_exp == 'logmel' or features_exp == 'mel':
        ...
        feature_extractor = audio_feature_extractor.LogMelExtractor(
            ...
            log=log, snv=snv)
    if features_exp =='wav2vec2':
        feat_dict = {spkID:featVec for spkID,featVec in kaldi_io.read_vec_flt_ark(config.WAV2VEC2_ARK) }
    if features_exp =='hubertLarge':
        feat_dict = {spkID:featVec for spkID,featVec in kaldi_io.read_vec_flt_ark(config.HUBERTLARGE_ARK) }
    # Loop through all the files and get the sampling rate, number of
    # samples and the time in minutes
    num_samples_feature = []
    num_feats = config.EXPERIMENT_DETAILS['FREQ_BINS']
    for pointer, folder in enumerate(labels[0]):
        ...
        if features_exp =='wav2vec2' or features_exp =='hubertLarge':
            feat_reshaped = feat_dict[str(folder)]
            feat_shape = int(feat_reshaped.shape[0]/num_feats)
            print('Folder Name: ', folder, ' dimensions are: ', feat_shape, num_feats)
            main_logger.info(
                f"Successfully created {features_exp} for the "
                f"audio file at: {folder}, it's dimensions "
                f"are: {feat_shape}, "
                f"{num_feats}")
            num_samples_feature.append(feat_shape)
        if features_exp =='compare16':
            feat_path = config.COMPARE16_FOLDER+'/'+str(folder)+'_P_audio_data.csv'
            feat_df = pd.read_csv(feat_path, sep=';')
            feat_df = feat_df.drop(['name', 'frameTime'], axis=1)
            # feat_df = feat_df.drop(feat_df.filter(regex='_de').columns, axis=1)
            feat_mat = feat_df.values
            height, width = feat_mat.shape
        ...
        elif features_exp == 'rasta_plp':
            mel_spec = rastaplp(updated_file, fs = 16000, win_time = 0.064, hop_time = 0.032, dorasta = True, modelorder = 8)
