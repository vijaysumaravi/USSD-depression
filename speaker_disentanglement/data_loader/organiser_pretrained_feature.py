...
def get_updated_features(data, min_samples, meta, segment_dim, freq_bins,
    ...
    reshaped_data = np.reshape(data, (int(data.shape[0]/(config.EXPERIMENT_DETAILS['PRETRAIN_STEPS']*config.EXPERIMENT_DETAILS['PRETRAIN_DIMS'])), config.EXPERIMENT_DETAILS['PRETRAIN_STEPS'], config.EXPERIMENT_DETAILS['PRETRAIN_DIMS']))
    import pdb
    #map into audio file
    ...
    #                                      feature_exp,
    #                                      segment_dim)
    new_meta_data = afe.feature_segmenter_pretrained_feature(reshaped_data,
                                          meta,
                                          feature_exp,
                                          segment_dim)
...
def process_data(freq_bins, features, labels, mode_label, min_samples, logger,
    ...
    if mode_label == 'train' and config.EXPERIMENT_DETAILS['CROP']:
        ...
        for pointer, i in enumerate(labels[0, :]):
            ...
            #                               segment_dim,
            #                               freq_bins)
            reshaped_i = np.reshape(i, (int(i.shape[0]/(config.EXPERIMENT_DETAILS['PRETRAIN_STEPS']*config.EXPERIMENT_DETAILS['PRETRAIN_DIMS'])), config.EXPERIMENT_DETAILS['PRETRAIN_STEPS'], config.EXPERIMENT_DETAILS['PRETRAIN_DIMS']))
    ...
    #                           dtype=np.float32)
    import pdb
    update_features = np.zeros((length, config.EXPERIMENT_DETAILS['PRETRAIN_DIMS'], config.EXPERIMENT_DETAILS['PRETRAIN_STEPS']),dtype = np.float32)
    pointer = 0
    locator = []
    final_folders = []
    final_classes = []
...
def get_lengths(data, labels, config):
    ...
    for p in range(len(labels[0])):
        ...
        elif exp_type == 'contentvec' or exp_type == 'wavlm' or exp_type == 'wavlm+' or exp_type == 'whisper_base':
