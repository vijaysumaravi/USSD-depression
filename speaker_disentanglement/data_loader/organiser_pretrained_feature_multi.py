...
def get_updated_features(data, min_samples, meta, segment_dim, freq_bins,
    ...
    reshaped_data = np.reshape(data, (int(data.shape[0]/(config.EXPERIMENT_DETAILS['PRETRAIN_STEPS']*config.EXPERIMENT_DETAILS['PRETRAIN_DIMS']*config.EXPERIMENT_DETAILS['pretrained_layer'])), config.EXPERIMENT_DETAILS['PRETRAIN_STEPS'], config.EXPERIMENT_DETAILS['PRETRAIN_DIMS'], config.EXPERIMENT_DETAILS['pretrained_layer']))
    import pdb
    #map into audio file
    #folder_name = str(meta[0])
    #audio_parent_dir = '/data/jinhan/Dataset/daic-woz-old/audio_feats/feats_DepAudioNet/mel_snv_exp/audio_data/'
    #audio_path = os.path.join(audio_parent_dir,folder_name+'_P_audio_data.npy')
    #audio_data = np.load(audio_path)
    
    def mel2audio(i):
        return config.HOP_SIZE * i, config.HOP_SIZE * i + config.WINDOW_SIZE

    if mode_label == 'train' and config.EXPERIMENT_DETAILS['CROP']:
        subsample_length = reshaped_data.shape[0] - min_samples
        if subsample_length < 0:
            ...
            #start, _ = mel2audio(random_pointer)
            #_, end = mel2audio(random_pointer + min_samples-1)
            #sampled_audio = audio_data[start:end]
    # tuple in form of features, folders, classes, scores, genders, indices
    #new_meta_data = afe.feature_segmenter(reshaped_data,
    #                                      meta,
    #                                      feature_exp,
    #                                      segment_dim)
    new_meta_data = afe.feature_segmenter_pretrained_feature_multi(reshaped_data,
                                          meta,
                                          feature_exp,
                                          segment_dim)
    #new_meta_data = afe.feature_segmenter_with_audio(reshaped_data, meta, feature_exp, segment_dim, sampled_audio, config)
...
def process_data(freq_bins, features, labels, mode_label, min_samples, logger,
    ...
    if mode_label == 'train' and config.EXPERIMENT_DETAILS['CROP']:
        ...
        for crop_length in min_samples:
            ...
            #length += temp_length
            temp_length = min_samples[crop_length][0]
            temp_length *= min_samples[crop_length][1]
            length += temp_length
        ...
        for pointer, i in enumerate(labels[0, :]):
            ...
            #                               segment_dim,
            #                               freq_bins)
            reshaped_i = np.reshape(i, (int(i.shape[0]/(config.EXPERIMENT_DETAILS['PRETRAIN_STEPS']*config.EXPERIMENT_DETAILS['PRETRAIN_DIMS']*config.EXPERIMENT_DETAILS['pretrained_layer'])), config.EXPERIMENT_DETAILS['PRETRAIN_STEPS'], config.EXPERIMENT_DETAILS['PRETRAIN_DIMS'], config.EXPERIMENT_DETAILS['pretrained_layer']))
            temp_length = reshaped_i.shape[0]
            length += temp_length
    
    #update_features = np.zeros((length, freq_bins, segment_dim),
    #                           dtype=np.float32)
    import pdb
    update_features = np.zeros((length, config.EXPERIMENT_DETAILS['PRETRAIN_DIMS'], config.EXPERIMENT_DETAILS['PRETRAIN_STEPS']), config.EXPERIMENT_DETAILS['pretrained_layer'],dtype = np.float32)
    pointer = 0
    locator = []
    final_folders = []
    final_classes = []
...
def data_info(labels, mode_label, logger, config, hidden_test=False):
    ...
    if mode_label == 'train' and config.EXPERIMENT_DETAILS['SUB_SAMPLE_ND_CLASS']:
        #min_set = min(len(zeros_index), len(ones_index))
        
        #consider gender
        zeros_g, zeros_index_g, ones_g, ones_index_g, indices_g = util.count_classes_gender(labels)
        zeros_index_f_g, zeros_index_m_g = zeros_index_g
        ones_index_f_g, ones_index_m_g = ones_index_g
        min_set = min(len(zeros_index_f_g), len(zeros_index_m_g),
                   len(ones_index_f_g), len(ones_index_m_g))
        min_set *= 2
...
def get_lengths(data, labels, config):
    ...
    for p in range(len(labels[0])):
        ...
        elif exp_type == 'contentvec' or exp_type == 'wavlm' or exp_type == 'wavlm+':
            d = np.reshape(d,  (int(d.shape[0]/(config.EXPERIMENT_DETAILS['PRETRAIN_STEPS']*config.EXPERIMENT_DETAILS['PRETRAIN_DIMS'])), config.EXPERIMENT_DETAILS['PRETRAIN_STEPS'], config.EXPERIMENT_DETAILS['PRETRAIN_DIMS']))
            d = np.transpose(d, (1,2,0))
        
        elif exp_type == 'wavlm_multi' or exp_type == 'wavlm+_multi':
            d = np.reshape(d,  (int(d.shape[0]/(config.EXPERIMENT_DETAILS['PRETRAIN_STEPS']*config.EXPERIMENT_DETAILS['PRETRAIN_DIMS']*config.EXPERIMENT_DETAILS['pretrained_layer'])), config.EXPERIMENT_DETAILS['PRETRAIN_STEPS'], config.EXPERIMENT_DETAILS['PRETRAIN_DIMS'], config.EXPERIMENT_DETAILS['pretrained_layer']))
