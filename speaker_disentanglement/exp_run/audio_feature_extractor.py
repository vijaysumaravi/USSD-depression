...
def feature_segmenter_pretrained_feature(feature, meta, feature_exp, dim, convert_to_image=False):
    """
    Segments the features into dimensions specified by feature.shape[-1] and
    dim. The number of extra dimensions is used to create lists of the
    folder, class, score and index for this updated reshaped data array
    ...
    #    assert False
    import pdb
    new_features = np.transpose(feature,(0,2,1))
    num_extra_dimensions = feature.shape[0]
    new_indexes = np.arange(num_extra_dimensions)
...
def feature_segmenter_pretrained_feature_multi(feature, meta, feature_exp, dim, convert_to_image=False):
    """
    Segments the features into dimensions specified by feature.shape[-1] and
    dim. The number of extra dimensions is used to create lists of the
    folder, class, score and index for this updated reshaped data array
...
def feature_segmenter_with_audio(feature, meta, feature_exp, dim, sampled_audio, config, convert_to_image=False):
    """
    Segments the features into dimensions specified by feature.shape[-1] and
    dim. The number of extra dimensions is used to create lists of the
    folder, class, score and index for this updated reshaped data array
    ...
    Inputs:
        ...
        feature_exp: Type of feature experiment eg. logmel
        dim: Value to segment the data by
        sampled_audio: audio array corresponds to features
        convert_to_image: Bool - Is the feature array being converted to 3D?
    ...
    duration_sample = mel2audio(119)[1] - mel2audio(0)[0]

    new_audio_array = np.zeros([num_extra_dimensions,duration_sample])
    ...
    for i in range(num_extra_dimensions):
        mel_index = i * 120
        if mel2audio(mel_index)[0] + duration_sample <= len(sampled_audio):
            audio_clip = sampled_audio[mel2audio(mel_index)[0]: mel2audio(mel_index)[0] + duration_sample]    
            assert mel2audio(mel_index + 119)[1] == mel2audio(mel_index)[0] + duration_sample
        else:
            audio_clip = np.hstack((sampled_audio[mel2audio(mel_index)[0]:], np.zeros((mel2audio(mel_index)[0] + duration_sample - len(sampled_audio),)))) 
    new_audio_array[i, :] = audio_clip
