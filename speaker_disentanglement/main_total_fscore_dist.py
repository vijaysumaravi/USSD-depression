...
# organiser: original organiser: mel, raw, wav2vec2
#organiser_pretrained_feature: contentvec, wavlm, wavlm+, whisper_base
#organiser_pretrained_Feature_multi: wavlm_multi, wavlm+_multi <TODO>
from data_loader import organiser as organiser
# from data_loader import organiser_pretrained_feature as organiser
#from data_loader import organiser_pretrained_feature_multi as organiser
...
# compare16 cnn_lstm
from exp_run.models_pytorch import CustomComparE16 as CustomComparE
from exp_run import config_compare16_cnn_lstm as config
...
def forward(model, generate_dev, data_type):
    ...
    for data in generate_dev:
        ...
        with torch.no_grad():
            batch_output, batch_embd = get_output_from_model(model=model,
                                                 data=batch_data)
        counter += 1
        # Append data
        ...
        embeddings.append(batch_embd.data.cpu().numpy())
    ...
    results_dict['folder'] = np.array(folders) #spk groundtruth labels
    targets = np.concatenate(targets, axis=0)
    results_dict['target'] = np.array(targets)
    embeddings = np.concatenate(embeddings, axis = 0)
    results_dict['embeddings'] = np.array(embeddings)  #spk features
...
def write_embeddings(results_dict):
    now = datetime.datetime.now()
    test_id = now.strftime("%Y%m%d%H%M%S%f")
    dir_path = os.path.join(features_dir, sub_dir+'_test','embeddings_'+threshold)
    if not os.path.exists(dir_path):
        os.mkdir(dir_path)
    embedding_ark_path = os.path.join(dir_path, test_id+'.pkl')
    
    print(f"Writing Embeddings to {embedding_ark_path}")
    folders_list = results_dict['folder']
    embeddings_list = results_dict['embeddings']
    embd_dict = {k:[] for k in np.unique(folders_list)}
    
    for key, value in zip(folders_list, embeddings_list):
        embd_dict[key].append(value)
    
    with open(embedding_ark_path, 'wb') as f:
...
def create_model():
    ...
    elif config.EXPERIMENT_DETAILS['FEATURE_EXP'] in ['contentvec' , 'wavlm' , 'wavlm+', 'wav2vec2', 'whisper_base']:
        model = CustomMel()
    elif config.EXPERIMENT_DETAILS['FEATURE_EXP'] in ['compare16', 'compare16_delta']:
        model = CustomComparE()
...
def setup(current_dir, model_dir, exp_num,data_type='', path_to_logger_for_test=None):
    ...
    if os.path.exists(current_dir) and os.path.exists(model_dir) and mode != 'train':
        ...
        if threshold == 'total':
            ...
            with open(model_dir + '/best_scores.pickle','rb') as f:
                current_epoch = pickle.load(f)[-1]
...
def get_output_from_model(model, data):
    ...
    output,  output_embd = model(current_data)
    if cuda:
        output = output.cpu()
        output_embd = output_embd.cpu()

    return output, output_embd
...
def train(model, workspace_files_dir):
    ...
    train_folders.sort()    
    # read speaker embeddings
    speaker_embeddings = mu.read_speaker_embeddings(config.spk_embd_path)
    if checkpoint:
    ...
    for (iteration, (batch, train_batch_loc, data_saver)) in enumerate(
        ...
        batch_spk_embd = mu.get_batch_speaker_embeddings(batch_folders,
                                                         speaker_embeddings)

        train_loss_mdd = mu.calculate_loss(batch_output,
                                       batch_labels,
                                       batch_weights,
                                       gender_balance)


        train_loss_spk = mu.calculate_speaker_loss_dist(batch_embd,batch_spk_embd)
        train_loss_spk = config.EXPERIMENT_DETAILS['LOSS_ALPHA'] * train_loss_spk
        ...
        #1:1
        train_loss = train_loss_mdd + train_loss_spk
        optimizer.zero_grad()
        train_loss.backward()
        optimizer.step()
    ...
    for exp_num in range(exp_runthrough):
        ...
        if config.EXPERIMENT_DETAILS['SPLIT_BY_GENDER']:
            ...
            scores, per_epoch, results_dict = evaluate_val_test(model,
                                         ...
                                         gender_balance,
                                         hidden_test)
            results_dict_dir = os.path.join(current_dir, 'train_results_dict_'+args.threshold)
...
comp_scores_avg[1],comp_scores_avg[2],comp_scores_avg[3],comp_scores_avg[4],comp_scores_avg[5],comp_scores_avg[6],comp_scores_avg[7],comp_scores_avg[8],comp_scores_avg[9],))
    ...
    if data_type == 'test' or data_type=='dev':
        if config.EXPERIMENT_DETAILS['SPLIT_BY_GENDER']:
            ...
            accum_results_dict_path = os.path.join(current_dir, 'accum_results_dict_'+args.threshold+'.pickle')
            with open(accum_results_dict_path,'wb') as f:
...
comp_scores[1],comp_scores[2],comp_scores[3],comp_scores[4],comp_scores[5],comp_scores[6],comp_scores[7],comp_scores[8],comp_scores[9],))
            ...
            result_pred_target_file = os.path.join(current_dir, 'result_pred_target_'+args.threshold+'.dict')
            with open(result_pred_target_file,'wb') as f:
                pickle.dump(results,f) 
            if args.alpha != 0:
                print('Using alpha {}'.format(config.EXPERIMENT_DETAILS['LOSS_ALPHA']))
                main_logger.info('Using alpha {}'.format(config.EXPERIMENT_DETAILS['LOSS_ALPHA']))

            if args.lr != 0:
                print('Using lr {}'.format(config.EXPERIMENT_DETAILS['LEARNING_RATE']))
                main_logger.info('Using lr {}'.format(config.EXPERIMENT_DETAILS['LEARNING_RATE']))
...
if __name__ == '__main__':
    ...
    parser_train.add_argument('--lr',
                              type=float,
                              default=0,
                              help='Learning Rate, higher priority than config, use config lr by not setting this')   
 
    parser_train.add_argument('--alpha',
                              type=float,
                              default=0,
                              help='disentanglement loss coefficient, higher priority than config, use config alpha by not setting this')
    ...
    if args.alpha != 0:
       config.EXPERIMENT_DETAILS['LOSS_ALPHA'] = args.alpha    

    if args.lr != 0:
       config.EXPERIMENT_DETAILS['LEARNING_RATE'] = args.lr
    ...
    #the following sub_dir change should be compatiable with sub_dir format
  
    if args.lr != 0:
        if config.EXPERIMENT_DETAILS['LOSS_ALPHA'] == 0:
            elements = sub_dir.split('_')
            elements[-5] = str(args.lr)
            sub_dir = '_'.join(elements)
        else:
            elements = sub_dir.split('_')
            elements[-7] = str(args.lr)
            sub_dir = '_'.join(elements)

    if args.alpha != 0:
        elements = sub_dir.split('_')
        elements[-1] = str(args.alpha)
        sub_dir = '_'.join(elements)
 
    config.EXPERIMENT_DETAILS['SUB_DIR'] = sub_dir
